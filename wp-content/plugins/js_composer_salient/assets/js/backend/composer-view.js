...
(function ( $ ) {
	...
	var VisualComposer = vc.visualComposerView = Backbone.View.extend( {
		...
		render: function () {
			...
			if ( this.accessPolicy !== 'only' ) {
                ...
                //portfolio extra
                     ...
                     this.$buttonsContainer = $('<div class="composer-switch"><span class="logo-icon"></span><span class="vc_spacer"></span><a class="wpb_switch-to-composer" href="#">' + window.i18nLocale.main_button_title_backend_editor + '</a>' + front + '</div>').prependTo('#normal-sortables');      
                }
			    ...
			    //set current post to portfolio extra if applicable
            if($('#nectar-metabox-portfolio-extra').length == 0){ 
                this.$post = $('#postdivrich');
            } 
            //portfolio extra
		...
		addElement: function ( e ) {
			...
			$('body').append('<div class="modal-backdrop white"></div>');
            //$('.wpb-content-layouts-container li').each(function(i){
            //    $(this).delay(i*16).animate({'opacity':1},300);
           // });
